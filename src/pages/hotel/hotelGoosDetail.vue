<template>
	<div class="hotel-goods">
		...
		<div class="goods-banner">
			...
			<!-- 计页数 -->
			<!-- <label class="count">1/8</label> -->
		</div>
		<!-- 商品名 -->
		<div class="goods-all">
			<div class="goods-name">
				<div class="money-all ">
					<label class="total-money"><span>¥</span>{{shopData.retailAmount}}</label>
				</div>
			...
			<div class="goods-pro">
				<div class="couple">
					...
					<div class="tr" v-if="shopData.coupons >0 ">领券<img class="icon-return" src="../../../static/img/icon-r-return.png" /></div>
					<div class="tr" v-else>暂无优惠券</div>
				</div>
				<div class="select">
					<div class="tl">房型</div>
					<div class="tr c1e select-txt">{{shopData.goodsName}}</div>
				</div>
				<div class="couple">
					<div class="tl">时间</div>
					<div class="tr flex2"   @click="toDate()">
						<label v-show="timeState==false">请选择入住时间</label>
						<label  v-show="timeState==true">{{starTime}}~{{endTime}}</label>
						<img class="icon-return" src="../../../static/img/icon-r-return.png" /></div>
				</div>
			</div>
			<!-- 商品评价 -->
		...
		<!-- 购物导航-->
		<div class="car-foot">
			<div class="car-icon" @click="tocompany(shopData.merchantId)"><img class="goods-home" src="../../../static/img/goods-home.png" /></div>
			<div class="car-icon"><a :href="'tel:'+shopData.serviceIphone"><img class="call-phone" src="../../../static/img/call-phone.png" /></a></div>
			<div class="join-car">¥<label>{{shopData.retailAmount}}</label></div>
	...
	import message from '../../components/message.vue';
		...
		export default {
			...
			mounted(){
				...
				this.timeState = this.$route.query.timeState || false;
				this.starTime = this.$route.query.startTime;
			...
			methods: {
				...
				// 详情
				detail(){
					hotel.detail(this.params).then(res =>{
						let code = res.code;
						if(code == 0){
					...
					if(this.timeState){
						this.$router.push({
							...
							query:{
								...
								specId:this.shopData.goodsSpecVo[0].id,
								kind:4
							}
						})
					}else{
