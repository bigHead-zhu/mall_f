<template>
	<div class="date-main">
		<div class="header">
			...
			<span class="ok-btn" @click="toHotel">完成</span>
		</div>
		<ul class="week-title">
					...
					<ul class="date-table clear">
						<li  class="f28  "  v-for="(dayobject,indexo) in item.dayArr" :key="indexo" >
							<label  @click="selectDate($event,index,indexo,dayobject.day.getDate())"
								:class="[{'cffb':dayobject.isWeek,'caaa':dayobject.showState == 0,'cle':dayobject.isWeek==false,'bgffb':dayobject.showState == 2 || dayobject.showState == 4,'bgff9':dayobject.showState == 3}]">
		...
		methods: {
			...
			toHotel(){
				this.$router.push({
					...
					query:{
						endTime:this.neeEndTime,
						startTime:this.neeStartTime,
						timeState:true,
						id :this.id
					}
				})
					...
					if (spindex && sindexc) {
						...
						if (spindex == pindex) {
							if (sindexc <= index) {
								...
								// 返回时间
								this.neeEndTime = this.days[pindex].dateTile + "-" + this.days[pindex].dayArr[index].day.getDate()
								this.neeStartTime  = this.days[spindex].dateTile + "-" + this.days[spindex].dayArr[sindexc].day.getDate()
								...
								// 返回时间
								this.neeStartTime = this.days[pindex].dateTile + "-" + this.days[pindex].dayArr[index].day.getDate()
								this.neeEndTime = this.days[spindex].dateTile + "-" + this.days[spindex].dayArr[sindexc].day.getDate()
