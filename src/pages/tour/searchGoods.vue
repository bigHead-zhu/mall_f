<template>
	<div class="tour-main-goods-search">
		<div class="header">
			<div class="searchs-content">
				...
				<div class="search">
					...
					<input class="search-input" type="text" placeholder="搜索商品" v-model="searchTitle"  readonly="readonly" @click="tosearch"/>
				</div>
			</div>
			<!-- 排序 -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			getSearchData() {
				// 搜索
				search.searchTour(this.params).then(res => {
					console.log(res)
					let code = res.code;
					if(code==0){
						if(res.data.count > 0){
							...
							this.searchGoodsData = this.searchGoodsData.concat(res.data.tourismVo);
